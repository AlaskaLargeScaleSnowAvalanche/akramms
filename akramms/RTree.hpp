/*
 * Taken from IBMisc: Misc. Routines for IceBin (and other code)
 */

#ifndef RTREE_H
#define RTREE_H

// See: https://github.com/nushoin/RTree
...
protected:
  ...
  bool Search(Node* a_node, Rect* a_rect, int& a_foundCount, RTree::Callback const &a_resultCallback);
...
int RTREE_QUAL::Search(std::array<ELEMTYPE, NUMDIMS> const &a_min, std::array<ELEMTYPE, NUMDIMS> const &a_max, RTree::Callback const &a_resultCallback)
{
  ...
  int foundCount = 0;
  Search(m_root, &rect, foundCount, a_resultCallback);
...
RTREE_TEMPLATE
bool RTREE_QUAL::Search(Node* a_node, Rect* a_rect, int& a_foundCount, RTree::Callback const &a_resultCallback)
{
  ASSERT(a_node);
  ASSERT(a_node->m_level >= 0);
  ASSERT(a_rect);
  ...
  if(a_node->IsInternalNode()) // This is an internal node in the tree
  {
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
        if(!Search(a_node->m_branch[index].m_child, a_rect, a_foundCount, a_resultCallback))
        {
