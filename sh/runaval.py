...
print(f'======= Running runaval.py in directory {os.getcwd}')

#EXE_EXT = '.exe' if os.name=='nt' else ''
#ramms_aval_LHM = config.roots['HARNESS'] / 'rammscore' / 'build' / f'ramms_aval_LHM{EXE_EXT}'

ramms_aval_LHM = sys.argv[1]    # Name of RAMMS executable to run
base = sys.argv[2]    # Name of input/output files (in current dir)

#AVAL_DIR = os.getcwd()    # HTCondor sets this
AVAL_DIR = pathlib.Path('.')

av3_file = AVAL_DIR / f'{base}.av3'
log_base = os.path.join(AVAL_DIR, base)
...
# --------------------------------------------
# Unzip the files
in_zip = os.path.join(AVAL_DIR, f'{base}.in.zip')
...
try:
    ...
    if True:
        cmd = [ramms_aval_LHM, av3_file, f'{log_base}.out']
