...
import configparser
  ...
  ### python module_3.py <file.nc> <output_base>
...
def readWRF(ncFile, variable):
  ...
  dataset.close()
  dataset = nc.Dataset('/center1/DYNDOWN/rtladerjr/WPS-4.0/cfsr/4km/geo_southeast.nc', 'r')
...
def wrf2geotiff(ncFile, outBase):
  ...
  ### Maximum 3-day snowfall
  outFile = outBase+'_sx3.tif' ### Check variable name
  print('Converting Maximum 3-day snowfall to GeoTIFF (%s)'\
    % outFile)
  (data, geoTrans, proj) = readWRF(ncFile, 'sx3') ### Check variable name
